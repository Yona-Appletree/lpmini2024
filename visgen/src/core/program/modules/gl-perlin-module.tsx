...
export const GlPerlinModule = defineGlModule(
  "gl-perlin",
  {
    label: "Perlin Noise",
    params: RecordDef({
      color: Vec4Def({ default: [1, 1, 1, 1] }),
      scale: FloatDef({ default: 5.0 }),
  ...
  glsl`
    ...
    vec3 warp(vec3 pos) {
        vec3 q = vec3(
            fbm(pos + vec3(0.0, 1.0, 1.0)),
            fbm(pos + vec3(1.0, 0.0, 1.0)),
            fbm(pos + vec3(1.0, 1.0, 0.0))
        );
    ...
    float enhance(float x) {
        ...
        // Apply sharpness (power function)
        x = pow(x, uSharpness);
    ...
    void main() {
        ...
        // Generate warped noise value
        vec3 warped = warp(vec3(uv * uScale, uTimeOffset));
