...
use alloc::format;
use engine_core::lpscript::vm::{LpsProgram, FunctionDef};
use engine_core::lpscript::vm::opcodes::LpsOpCode;
...
pub fn program_to_lpa(program: &LpsProgram) -> String {
    ...
    output.push_str(&format!("; Generated by lps-compile\n\n"));
...
fn function_to_lpa(func: &FunctionDef, _func_idx: usize, program: &LpsProgram) -> String {
    ...
    // Function header
    output.push_str(&format!("function {} {}(", type_to_string(&func.return_type), func.name));
    ...
    if !func.locals.is_empty() {
        ...
        for local in &func.locals {
            output.push_str(&format!("  ;   {} {}\n", type_to_string(&local.ty), local.name));
        }
        output.push('\n');
    }
    ...
    for (pc, opcode) in func.opcodes.iter().enumerate() {
        output.push_str(&format!("  {:04}  {}\n", pc, opcode_to_pretty_string(opcode, func, program)));
    }
...
fn opcode_to_pretty_string(opcode: &LpsOpCode, func: &FunctionDef, program: &LpsProgram) -> String {
    use engine_core::lpscript::vm::opcodes::LoadSource;
    ...
    match opcode {
        ...
        LpsOpCode::LoadLocalFixed(idx) => {
            let local_name = func.locals.get(*idx as usize)
                .map(|l| l.name.as_str())
                .unwrap_or("?");
        ...
        LpsOpCode::Call(func_idx) => {
            let func_name = program.function(*func_idx as usize)
                .map(|f| f.name.as_str())
                .unwrap_or("?");
...
mod tests {
    use super::*;
    use engine_core::lpscript::{parse_expr, parse_script};
    ...
    fn test_simple_expr_to_lpa() {
        ...
        assert!(lpa.contains("load builtin.xNorm"), "Should have load builtin.xNorm");
        assert!(lpa.contains("load builtin.yNorm"), "Should have load builtin.yNorm");
    ...
    fn test_vec3_return_type_in_lpa() {
        ...
        assert!(lpa.contains("function vec3 main"), "LPA should show vec3 return type:\n{}", lpa);
    }
    ...
    fn test_fixed_values_as_floats() {
        ...
        assert!(!lpa.contains("Fixed(163840)"), "Should not show raw int 163840 for 2.5");
        assert!(!lpa.contains("(163840)"), "Should not show raw int representation");
    }
    ...
    fn test_pretty_assembly_format() {
        ...
        assert!(lpa.contains("push 1.5"), "Should have lowercase 'push', got:\n{}", lpa);
        assert!(lpa.contains("call add"), "Should have 'call add', got:\n{}", lpa);
        
        // Should use readable load sources
        assert!(lpa.contains("load builtin.time") || lpa.contains("load local."), 
                "Should have readable load sources, got:\n{}", lpa);
        
        // Should not have ugly Debug format
        assert!(!lpa.contains("Push("), "Should not have Debug format Push()");
        assert!(!lpa.contains("Call("), "Should not have Debug format Call()");
    }
    ...
    fn test_pretty_builtin_loads() {
        ...
        assert!(lpa.contains("load builtin.xNorm"), "Should show builtin.xNorm, got:\n{}", lpa);
        assert!(lpa.contains("load builtin.yNorm"), "Should show builtin.yNorm, got:\n{}", lpa);
        assert!(lpa.contains("load builtin.time"), "Should show builtin.time, got:\n{}", lpa);
    }
}
