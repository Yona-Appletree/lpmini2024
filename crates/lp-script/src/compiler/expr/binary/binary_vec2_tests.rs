...
mod tests {
    ...
    fn test_vec2_modulo() -> Result<(), String> {
        ExprTest::new("vec2(10.0, 7.0) % vec2(3.0, 2.0)")
            ...
            .expect_result_vec2(Vec2::new(1.0.to_fixed(), 1.0.to_fixed()))
            .run()
    }
    ...
    // Vec2 * Int32 scalar (should promote Int32 to Fixed)
    #[test]
    ...
    fn test_edge_cases() -> Result<(), String> {
        ...
        // Negative components
        ExprTest::new("vec2(-1.0, -2.0) + vec2(3.0, 4.0)")
            .expect_result_vec2(Vec2::new(2.0.to_fixed(), 2.0.to_fixed()))
            .run()?;

        // Multiply by zero
        ExprTest::new("vec2(1.0, 2.0) * 0.0")
            .expect_result_vec2(Vec2::new(0.0.to_fixed(), 0.0.to_fixed()))
            .run()
    }
}
