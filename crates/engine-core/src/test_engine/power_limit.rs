...
mod tests {
    ...
    fn test_power_calculation() {
        ...
        let gray_power =
            calculate_led_power(gray, config.led_white_power_ma, config.led_idle_power_ma);
        let expected = (config.led_idle_power_ma + config.led_white_power_ma) / 2;
        assert!(
            (gray_power as i32 - expected as i32).abs() < 3,
            "Gray power {} should be close to {}",
            gray_power,
            expected
        );
    }
    ...
    fn test_power_limiting_with_clamp() {
        ...
        assert!(
            total_power <= config.power_budget_ma,
            "Total power {} should be <= budget {}",
            total_power,
