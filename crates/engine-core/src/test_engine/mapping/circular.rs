...
impl LedMapping {
    ...
    /// Rings are wired from innermost to outermost for ring7 (legacy) 
    /// and for ring9 they are outermost to innermost. (amzn stock product wiring)
    ///
    /// # Arguments
    ...
    pub fn circular_panel_9ring(width: usize, height: usize) -> Self {
        ...
        for (ring_array_idx, &led_count) in ring_counts.iter().enumerate() {
            ...
            // Calculate radius for this ring in fixed-point
            // ring_array_idx 0 is outermost, ring_array_idx 9 is innermost
...
mod tests {
    ...
    fn test_circular_panel_9ring_led_counts() {
        ...
        for (ring_num, &expected_count) in ring_counts.iter().enumerate() {
            ...
            if ring_num == 0 {
                assert!(
                    count == expected_count || (count < expected_count && led_idx >= 256),
                    "Outermost ring should have {} LEDs, but found {} (may be limited by 256 LED cap)",
                    expected_count,
                    count
                );
            }
