...
use crate::lp_script::vm::{CallStack, ValueStack};
use crate::lp_script::{LocalStack, LpsProgram, LpsVmError, RuntimeErrorWithContext};
...
pub struct LpsVm<'a> {
    ...
    pub(crate) current_fn_idx: usize, // Track which function we're executing
}
...
impl<'a> LpsVm<'a> {
    ...
    pub(crate) fn runtime_error(&self, error: LpsVmError) -> RuntimeErrorWithContext {
    ...
    pub(crate) fn current_function_len(&self) -> usize {
...
mod tests {
    ...
    #[test]
    fn test_vm_creation() {
        use crate::lp_script::parse_expr;
