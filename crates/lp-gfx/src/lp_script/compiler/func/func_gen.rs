...
use crate::lp_script::compiler::ast::FunctionDef as AstFunctionDef;
use crate::lp_script::compiler::codegen::{CodeGenerator, LocalAllocator};
use crate::lp_script::compiler::error::CodegenError;
use crate::lp_script::compiler::func::{FunctionMetadata, FunctionTable};
use crate::lp_script::shared::Type;
use crate::lp_script::vm::opcodes::LpsOpCode;
use crate::lp_script::vm::{FunctionDef as VmFunctionDef, LocalVarDef, ParamDef};
...
pub fn gen_user_function(
    ...
    if !matches!(func_code.last(), Some(LpsOpCode::Return)) {
        if ast_func.return_type == Type::Void {
            ...
            func_code.push(LpsOpCode::Push(lp_math::dec32::Dec32::ZERO));
            func_code.push(LpsOpCode::Return);
        }
    }
