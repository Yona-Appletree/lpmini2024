...
impl TypeChecker {
    ...
    pub(crate) fn infer_type(
        ...
        match &mut expr.kind {
            ...
            ExprKind::Variable(name) => {
                let var_type = crate::lp_script::compiler::expr::variable::check_variable(
                    name, symbols, expr_span,
                )?;
            ...
            | ExprKind::Mod(left, right) => {
                let result_ty = crate::lp_script::compiler::expr::binary::check_binary_arithmetic(
                    left.as_mut(),
                    right.as_mut(),
                    symbols,
                    func_table,
            ...
            | ExprKind::PostDecrement(name) => {
                let ty = crate::lp_script::compiler::expr::variable::check_incdec(
                    name, symbols, expr_span,
                )?;
                expr.ty = Some(ty);
            }
            ...
            ExprKind::Call { name, args } => {
                let (ty, expanded_expr) = crate::lp_script::compiler::expr::call::check_call(
    ...
    fn check_bitwise_binary(
        ...
        symbols: &mut SymbolTable,
        func_table: &FunctionTable,
        _span: crate::lp_script::shared::Span,
