...
fn expand(input: &DeriveInput) -> Result<TokenStream2, Error> {
    match &input.data {
        ...
        Data::Enum(_) | Data::Union(_) => Err(Error::new(
            input.ident.span(),
            "LpValue derive only supports structs",
        )),
    }
}
...
fn expand_struct(input: &DeriveInput, data: &DataStruct) -> Result<TokenStream2, Error> {
    if !input.generics.params.is_empty() {
        return Err(Error::new(
            input.generics.span(),
            "LpValue derive does not yet support generic parameters",
        ));
    }
    ...
    let fields = match &data.fields {
        ...
        Fields::Unnamed(_) | Fields::Unit => {
            return Err(Error::new(
                data.fields.span(),
                "LpValue derive currently supports only structs with named fields",
            ))
        }
    };
