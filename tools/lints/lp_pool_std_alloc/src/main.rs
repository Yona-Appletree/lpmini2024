...
const BANNED_MACROS: &[BannedMacro] = &[
    ...
    BannedMacro {
        name: "vec_from",
        message: "use pool-backed vectors instead of `vec_from!`",
    },
    BannedMacro {
        name: "vec_deque",
        message: "use pool-backed deques instead of `vec_deque!`",
    },
    BannedMacro {
        name: "format",
    ...
    BannedMacro {
        name: "lazy_static",
        message: "avoid heap allocations in lazy statics without pool",
    },
];
    ...
    for base in paths {
        ...
        for entry in walker {
            ...
            match syn::parse_file(&source) {
                Ok(parsed) => {
                    let mut visitor = PoolLintVisitor::new(file_path.clone(), &source);
...
impl<'a> PoolLintVisitor<'a> {
    fn new(file: Utf8PathBuf, source: &'a str) -> Self {
        Self {
            file,
            source,
...
impl<'ast> Visit<'ast> for PoolLintVisitor<'_> {
