...
mod tests {
    ...
    fn test_sqrt_basic() {
        ...
        assert!((result.to_f32() - 2.0).abs() < 0.01, "sqrt(4) should be ~2, got {}", result.to_f32());
        
        let nine = Fixed::from_i32(9);
        let result = sqrt(nine);
        assert!((result.to_f32() - 3.0).abs() < 0.01, "sqrt(9) should be ~3, got {}", result.to_f32());
    }
    ...
    fn test_sqrt_fractional() {
        ...
        assert!((result.to_f32() - expected).abs() < 0.01, 
                "sqrt(2) should be ~{}, got {}", expected, result.to_f32());
    }
    ...
    fn test_sqrt_small_values() {
        ...
        let expected = 0.5;
        assert!((result.to_f32() - expected).abs() < 0.01, 
                "sqrt(0.25) should be ~{}, got {}", expected, result.to_f32());
    }
}
