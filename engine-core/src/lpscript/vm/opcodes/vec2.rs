...
pub fn exec_add_vec2(stack: &mut [i32], sp: &mut usize) -> Result<(), RuntimeError> {
    ...
    stack[*sp - 4] = (Fixed(stack[*sp - 4]) + Fixed(stack[*sp - 2])).0; // a.x + b.x
    stack[*sp - 3] = (Fixed(stack[*sp - 3]) + Fixed(stack[*sp - 1])).0; // a.y + b.y
    *sp -= 2;
...
pub fn exec_neg_vec2(stack: &mut [i32], sp: &mut usize) -> Result<(), RuntimeError> {
    ...
    // Direct stack manipulation - no vector construction needed
    stack[*sp - 2] = (-Fixed(stack[*sp - 2])).0; // -v.x
    stack[*sp - 1] = (-Fixed(stack[*sp - 1])).0; // -v.y
...
pub fn exec_mul_vec2_scalar(stack: &mut [i32], sp: &mut usize) -> Result<(), RuntimeError> {
    ...
    stack[*sp - 3] = (Fixed(stack[*sp - 3]) * scalar).0; // v.x * scalar
    stack[*sp - 2] = (Fixed(stack[*sp - 2]) * scalar).0; // v.y * scalar
    *sp -= 1;
...
pub fn exec_dot2(stack: &mut [i32], sp: &mut usize) -> Result<(), RuntimeError> {
    ...
    let ax = Fixed(stack[*sp - 4]);
    let ay = Fixed(stack[*sp - 3]);
    let bx = Fixed(stack[*sp - 2]);
    let by = Fixed(stack[*sp - 1]);
...
pub fn exec_length2(stack: &mut [i32], sp: &mut usize) -> Result<(), RuntimeError> {
    ...
    // Direct computation - no vector construction needed
    let vx = Fixed(stack[*sp - 2]);
    let vy = Fixed(stack[*sp - 1]);
