...
pub fn exec_add_vec3(stack: &mut [i32], sp: &mut usize) -> Result<(), RuntimeError> {
    ...
    stack[*sp - 6] = (Fixed(stack[*sp - 6]) + Fixed(stack[*sp - 3])).0; // a.x + b.x
    stack[*sp - 5] = (Fixed(stack[*sp - 5]) + Fixed(stack[*sp - 2])).0; // a.y + b.y
    stack[*sp - 4] = (Fixed(stack[*sp - 4]) + Fixed(stack[*sp - 1])).0; // a.z + b.z
    *sp -= 3;
...
pub fn exec_neg_vec3(stack: &mut [i32], sp: &mut usize) -> Result<(), RuntimeError> {
    ...
    // Direct stack manipulation - no vector construction needed
    stack[*sp - 3] = (-Fixed(stack[*sp - 3])).0; // -v.x
    stack[*sp - 2] = (-Fixed(stack[*sp - 2])).0; // -v.y
    stack[*sp - 1] = (-Fixed(stack[*sp - 1])).0; // -v.z
...
pub fn exec_mul_vec3_scalar(stack: &mut [i32], sp: &mut usize) -> Result<(), RuntimeError> {
    ...
    stack[*sp - 4] = (Fixed(stack[*sp - 4]) * scalar).0; // v.x * scalar
    stack[*sp - 3] = (Fixed(stack[*sp - 3]) * scalar).0; // v.y * scalar
    stack[*sp - 2] = (Fixed(stack[*sp - 2]) * scalar).0; // v.z * scalar
    *sp -= 1;
...
pub fn exec_dot3(stack: &mut [i32], sp: &mut usize) -> Result<(), RuntimeError> {
    ...
    let ax = Fixed(stack[*sp - 6]);
    let ay = Fixed(stack[*sp - 5]);
    let az = Fixed(stack[*sp - 4]);
    let bx = Fixed(stack[*sp - 3]);
    let by = Fixed(stack[*sp - 2]);
    let bz = Fixed(stack[*sp - 1]);
...
pub fn exec_length3(stack: &mut [i32], sp: &mut usize) -> Result<(), RuntimeError> {
    ...
    // Direct computation - no vector construction needed
    let vx = Fixed(stack[*sp - 3]);
    let vy = Fixed(stack[*sp - 2]);
    let vz = Fixed(stack[*sp - 1]);
