...
#[cfg(test)]
mod mapping_tests {
    use crate::test_engine::{MappingConfig, apply_2d_mapping};
    ...
    fn test_circular_panel_led_count() {
        ...
        assert_eq!(led_count, 113, "CircularPanel7Ring should have 113 LEDs (1+8+12+16+20+24+32)");
    }
    ...
    fn test_mapping_gradient() {
        ...
        assert!(has_variation, "Mapping should produce varied output for gradient input");
    }
    ...
    fn test_different_sizes() {
        ...
        for size in [4, 8, 16, 32] {
            let config = MappingConfig::CircularPanel7Ring { width: size, height: size };
            let led_count = config.led_count();
