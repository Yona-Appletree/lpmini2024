...
use lp_script::math::{Fixed, FIXED_SHIFT};
...
/// Sample an RGB pixel from a 2D buffer at fixed-point coordinates
///
/// # Arguments
/// * `buffer` - RGB buffer (width * height * 3 bytes)
/// * `x` - X coordinate in fixed-point
/// * `y` - Y coordinate in fixed-point
